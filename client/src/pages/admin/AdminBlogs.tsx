    ...
    <div className="p-2 sm:p-6 space-y-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Manage articles, draft posts, and SEO details.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-10 text-center text-slate-500 bg-white dark:bg-dark-900/30 rounded-2xl border border-slate-200 dark:border-dark-700 shadow-sm font-medium">No blogs found.</div>
      ) : (
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-gray-700/50">
              {blogs.map((blog) => (
                <tr key={blog._id} className="hover:bg-slate-50/80 dark:hover:bg-gray-800/30 transition-colors">
                  <td className="p-4">
                    <img src={blog.coverImage || 'https://via.placeholder.com/150'} alt="cover" className="w-16 h-10 object-cover rounded-lg border border-slate-200 dark:border-dark-700" />
                  </td>
                  <td className="p-4 font-semibold text-slate-900 dark:text-white max-w-xs truncate">{blog.title}</td>
                  <td className="p-4 font-medium text-slate-600 dark:text-slate-300">{blog.category}</td>
                  <td className="p-4 font-bold text-slate-900 dark:text-slate-200">{blog.views}</td>
                  <td className="p-4">
                    <span className={blog.published ? 'badge-primary' : 'badge-orange'}>
                      {blog.published ? 'Published' : 'Draft'}
                    </span>
                  </td>
                  <td className="p-4 text-slate-500 font-medium">{formatDate(blog.createdAt)}</td>
